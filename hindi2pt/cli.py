...
def translate_file(source, translator, out_dir, log=print, raw=False, max_cps=20.0, offset=0.0, factor=1.0, bilingual=False):
...
def run_one(source, args, translator, log=print):
    ...
    srt = translate_file(path, translator, args.out, log=log, raw=args.raw, max_cps=args.max_cps,
                         offset=args.offset, factor=args.scale, bilingual=args.bilingual)
    audio = None
    if args.dub:
